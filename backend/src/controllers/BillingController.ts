...
export class BillingController {
   ...
   */
  static async getUserBilling(req: Request, res: Response): Promise<void> {
    try {
   ...
   */
  static async getAllUsersBilling(req: Request, res: Response): Promise<void> {
    try {
   ...
   */  
  static async addUserBalance(req: Request, res: Response): Promise<void> {
    try {
      if (req.user!.role !== 'ADMIN') {
        return res.status(403).json({ error: 'Admin access required' })
      }
   ...
   */
  static async setUserBalance(req: Request, res: Response): Promise<void> {
    try {
      if (req.user!.role !== 'ADMIN') {
        return res.status(403).json({ error: 'Admin access required' })
      }
   ...
   */
  static async deductUserBalance(req: Request, res: Response): Promise<void> {
    try {
      if (req.user!.role !== 'ADMIN') {
        return res.status(403).json({ error: 'Admin access required' })
      }
   ...
   */
  static async startPaidSubscription(req: Request, res: Response): Promise<void> {
    try {
      if (req.user!.role !== 'ADMIN') {
        return res.status(403).json({ error: 'Admin access required' })
      }
      ...
      res.status(500).json({ error: 'Failed to start paid subscription' })
    }
  }
   ...
   */
  static async triggerBillingManually(req: Request, res: Response): Promise<void> {
    try {
      if (req.user!.role !== 'ADMIN') {
        return res.status(403).json({ error: 'Admin access required' })
      }
